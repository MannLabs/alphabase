...
class Percolator:
    ...
    def feature_list(self) -> list:
        ...
        return list(set(self._base_features + self.feature_extractor.feature_list))
    ...
    def extract_features(self, psm_df: pd.DataFrame, *args, **kwargs) -> pd.DataFrame:
        ...
        *args and **kwargs are used for
        ...
        return self._feature_extractor.extract_features(psm_df, *args, **kwargs)

    def rescore(self, df: pd.DataFrame) -> pd.DataFrame:
        """
    ...
    def run_rerank_workflow(
        ...
        *args and **kwargs are used for
        ...
        """
        top_k_psm_df = self.extract_features(top_k_psm_df, *args, **kwargs)
        idxmax = top_k_psm_df.groupby(["raw_name", rerank_column])["ml_score"].idxmax()
    ...
    def run_rescore_workflow(
        self, psm_df: pd.DataFrame, *args, **kwargs
    ) -> pd.DataFrame:
        """
        ...
        *args and **kwargs are used for
        ...
        pd.DataFrame
            psm_df with feature columns appended inplace.
        """
        df = self.extract_features(psm_df, *args, **kwargs)
        return self.rescore(df)

    def _estimate_fdr_per_raw(self, df: pd.DataFrame, fdr_level: str) -> pd.DataFrame:
        df_list = []
    ...
    def _estimate_fdr(
        ...
        if per_raw_fdr:
            return self._estimate_fdr_per_raw(df, fdr_level=fdr_level)
    ...
    def _train(self, train_t_df: pd.DataFrame, train_d_df: pd.DataFrame):
        ...
        if len(train_t_df) > self.max_training_sample:
            train_t_df = train_t_df.sample(
                n=self.max_training_sample, random_state=1337
            )
        ...
        self._ml_model.fit(train_df[self.feature_list].values, train_label)
    ...
    def _train_and_score(self, df: pd.DataFrame) -> pd.DataFrame:
        df_target = df[df.decoy == 0]
        df_decoy = df[df.decoy != 0]

        if (
            np.sum(df_target.fdr <= self.training_fdr) < self.min_training_sample
            or len(df_decoy) < self.min_training_sample
        ):
            return df
    ...
    def _cv_score(self, df: pd.DataFrame) -> pd.DataFrame:
        ...
        if (
            ...
            or len(df_decoy) < self.min_training_sample * self.cv_fold
        ):
            return df
