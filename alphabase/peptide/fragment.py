...
__all__ = ['get_charged_frag_types', 'parse_charged_frag_type', 'get_delta_modification_mass',
           'get_b_y_and_peptide_mass', 'get_peptide_mass_for_same_len_seqs',
           'get_b_y_and_peptide_mass_for_same_len_seqs', 'init_zero_fragment_dataframe',
           ...
           'get_fragment_mass_dataframe', 'update_precursor_mz']
...
def get_delta_modification_mass(
    pep_len:int,
    ...
    mass deltas other than mod names.
    Args:
        pep_len (int): nAA
...
def get_b_y_and_peptide_mass(
    ...
    `get_b_y_and_peptide_mass_for_same_len_seqs()`
    ...
    if mass_deltas:
        mod_masses = get_delta_modification_mass(
...
def get_b_y_and_peptide_mass_for_same_len_seqs(
    sequences: np.array,
    ...
    if mass_delta_list is not None:
        for i, (mass_deltas, sites) in enumerate(zip(
            ...
            if len(mass_deltas) != 0:
                mod_masses[i,:] += get_delta_modification_mass(
                    seq_len,
                    mass_deltas,
                    sites,
                )
...
def update_precursor_mz(
    ...
    for nAA, df_group in _grouped:
        ...
        pep_mass = get_peptide_mass_for_same_len_seqs(
            df_group.sequence.values.astype('U'),
