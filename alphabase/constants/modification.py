...
__all__ = ['load_mod_yaml', 'load_modloss_importance', 'update_all_by_MOD_DF', 'add_modifications_for_lower_case_AA',
           'MOD_INFO_DICT', 'MOD_CHEM', 'MOD_MASS', 'MOD_LOSS_MASS', 'MOD_DF', 'MOD_formula', 'calc_modification_mass',
...
#: Dict loaded from used_mod.yaml.
# Note that we also added a "lower_case_AA" bool item
...
MOD_LOSS_MASS:dict = {}
#: pd.DataFrame structure of MOD_INFO_DICT.
MOD_DF:pd.DataFrame = pd.DataFrame()
#: Modification to formula dict of dict. {modname: {'C': n, 'H': m, ...}}
MOD_formula:dict = {}
...
def _update_all_by_MOD_INFO_DICT():
    ...
    for mod, val in MOD_INFO_DICT.items():
        ...
        MOD_formula[mod] = dict(parse_formula(val['composition']))
        MOD_LOSS_MASS[mod] = val['modloss']

def load_mod_yaml(yaml_file:str):
    """Load and init all global modification variables from a yaml file.

    Args:
        yaml_file (str): such as `used_mod.yaml` in _base_dir_
    """
