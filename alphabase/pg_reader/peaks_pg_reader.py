...
def _split_peaks_identifiers(df: pd.DataFrame) -> pd.DataFrame:
    ...
    a pipe character. This method splits these identifiers into separate columns.

    Parameters
    ----------
    df
        DataFrame containing PEAKS protein group data.

    Returns
    -------
    pd.DataFrame
        DataFrame with separate protein and gene columns.

    """
    df[[PGCols.PROTEINS, PGCols.GENES]] = df[ACCESSION_COLUMN].str.split(
        PROTEIN_GENE_SEPARATOR, expand=True
    )
    return df
