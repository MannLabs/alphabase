...
def _frags_for(speclib: PEAKSLibraryReader, precursor_mz: float):
    ...
    return speclib.fragment_df.iloc[
        row["flat_frag_start_idx"] : row["flat_frag_stop_idx"]
    ]
...
def test_no_modification_gives_empty_strings_not_nan_text(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 500.0].iloc[
        0
    ]
    assert row["mods"] == ""
...
def test_single_modification(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 510.0].iloc[
        0
    ]
...
def test_three_modifications_on_one_peptide(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 520.0].iloc[
        0
    ]
...
def test_n_terminal_modification_gets_fixed_site_zero(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 430.0].iloc[
        0
    ]
...
def test_modification_on_last_residue_site_equals_sequence_length(
    edge_case_precursor_df,
):
    """Off-by-one guard: PEAKS 0-based index 5 on a 6-residue peptide -> site "6"
    (== len(sequence)), not 5 and not 7.
    """
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 440.0].iloc[
        0
    ]
...
def test_two_modifications_at_the_same_position_are_both_kept(edge_case_precursor_df):
    ...
    """
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 450.0].iloc[
        0
    ]
...
def test_modifications_out_of_position_order_preserve_input_order(
    edge_case_precursor_df,
):
    ...
    """
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 460.0].iloc[
        0
    ]
    assert row["mods"] == "Carboxymethyl@C;Oxidation@M"
...
def test_trailing_stray_delimiter_is_ignored(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 470.0].iloc[
        0
    ]
...
def test_short_and_long_peptide_boundaries(edge_case_precursor_df):
    short_row = edge_case_precursor_df[
        edge_case_precursor_df["precursor_mz"] == 300.0
    ].iloc[0]
    long_row = edge_case_precursor_df[
        edge_case_precursor_df["precursor_mz"] == 900.0
    ].iloc[0]
...
def test_fragment_number_exceeding_peptide_length_raises_clear_error():
    ...
    with pytest.raises(ValueError, match="not chemically possible"):
        PEAKSLibraryReader().import_file(
            str(_DATA_DIR / "peaks_impossible_fragment.tsv")
        )
...
def test_rt_zero_is_not_treated_as_missing(edge_case_precursor_df):
    row = edge_case_precursor_df[edge_case_precursor_df["precursor_mz"] == 288.0].iloc[
        0
    ]
    assert row["rt"] == 0.0
...
def test_exact_duplicate_rows_are_both_kept(edge_case_precursor_df):
    ...
    assert len(rows) == 2
    assert (
        rows["flat_frag_start_idx"].nunique() == 2
    )  # each keeps its own fragment slice
